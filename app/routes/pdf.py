    ...
    try:
        ...
        for file in files:
            if not file.filename.lower().endswith('.pdf'):
                raise HTTPException(
                    status_code=400, detail="All files must be PDFs")
        ...
        timestamp = tempfile.mktemp(prefix='', dir='').lstrip(
        ...
        for temp_file in temp_files:
            try:
                ...
                logger.warning(f"Failed to delete temporary file {
                               temp_file}: {e}")
        ...
        for temp_file in temp_files:
            try:
                ...
                pass
        raise HTTPException(
            status_code=500, detail="Failed to combine PDF files")
...
async def compress_pdf(
    ...
    quality_lvl = quality_lvls[quality]

    logger.info(f"Compressing PDF file: {
                file.filename} Quality level: {quality}")
    ...
    try:
        ...
        # Read and compress
        writer = PdfWriter(clone_from=str(temp_path))
        ...
        for page in writer.pages:
            for img in page.images:
                img.replace(img.image, quality=quality_lvl['img_quality'])
        ...
        try:
            ...
            pass
        raise HTTPException(
            status_code=500, detail="Failed to compress PDF file")
    ...
    try:
        for file in TEMP_DIR.glob("*"):
            try:
                ...
                logger.warning(
                    f"Failed to delete old temporary file {file}: {e}")
    except Exception as e:
