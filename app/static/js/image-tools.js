class ImageHandler {
    ...
    setupPreviewContainer() {
        ...
        if (this.formId === "resize") {
            this.previewContainer = this.form.querySelector(
                ".image-preview-container"
            );
            this.previewImage =
                this.previewContainer.querySelector("#preview-image");
            ...
            this.widthDisplay =
                this.previewContainer.querySelector("#image-width");
            this.heightDisplay =
                this.previewContainer.querySelector("#image-height");
            return;
        }
    ...
    setupResizeHandles() {
        const handles =
            this.previewContainer.querySelectorAll(".resize-handle");
    ...
    resize(e) {
        ...
        if (this.currentHandle.classList.contains("bottom-right")) {
            ...
            newHeight = this.maintainAspect.checked
    ...
    async handleSubmit(e) {
        ...
        if (
            ...
            this.showFeedback(
                "error",
                "Please select an image file with a valid extension (jpg, jpeg, png, gif, or webp)."
            );
            return;
        }
        ...
        if (this.formId === "convert") {
            ...
            if (format === "jpeg" || format === "jpg") {
                try {
                    console.log(
                        "Checking image transparency before JPEG conversion..."
                    );
                    // Create a blob URL to ensure we can load the image
                    const objectUrl = URL.createObjectURL(file);
                    const hasTransparency = await new Promise(
                        (resolve, reject) => {
                            const img = new Image();
                            img.onload = () => {
                                try {
                    ...
                    if (hasTransparency) {
                        console.log(
                            "Transparency detected, preventing JPEG conversion"
                        );
                        this.showFeedback(
                            "error",
                            "Cannot convert images with transparency to JPEG. Please select PNG format instead."
                        );
                    ...
                    this.showFeedback(
                        "error",
                        "Error checking image transparency. Please try a different image."
                    );
                    return;
                }
            }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (errorData?.detail) {
                    if (
                        ...
                        errorMessage =
                            "Cannot convert image with transparency to JPEG. Please choose a different format like PNG.";
                    } else if (errorData.detail.includes("'latin-1' codec")) {
                        errorMessage =
                            "The filename contains special characters. Please rename the file using only basic letters and numbers.";
                    } else {
            ...
            if (blob.size === 0) {
                throw new Error(
                    "The processed image is empty. Please try again with a different image or settings."
                );
            }

            if (!blob.type.startsWith("image/")) {
                throw new Error(
                    "The server returned an invalid image format. Please try again."
                );
            }
            ...
            processedImage.onerror = () => {
                this.showFeedback(
                    "error",
                    "Failed to load processed image preview"
                );
        ...
        try {
            ...
            if (hasTransparency) {
                ...
                if (formatSelect) {
                    ...
                    this.showFeedback(
                        "error",
                        "This image contains transparency. JPEG format has been disabled as it does not support transparency. Please use PNG format instead."
                    );
                }
            }
        } catch (error) {
